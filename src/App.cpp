...
void App::mainLoop() {
    ...
    ImGui::DockSpaceOverViewport(ImGui::GetMainViewport()); // Get the display size

    ImVec2 displaySize = ImGui::GetIO().DisplaySize;

    // Calculate the window size (half the display width and height)
    ImVec2 windowSize(displaySize.x / 2, displaySize.y / 2);
    // if (show_menubar)
    //     renderMenuBar();
    ...
    ImGui::SetNextWindowPos(ImVec2(windowSize.x, 0), ImGuiCond_FirstUseEver);
    ImGui::SetNextWindowSize(windowSize, ImGuiCond_FirstUseEver);
    if (show_info)
        renderInfo();
    ...
    ImGui::SetNextWindowPos(ImVec2(0, windowSize.y), ImGuiCond_FirstUseEver);
    ImGui::SetNextWindowSize(windowSize, ImGuiCond_FirstUseEver);
    if (show_maze)
        renderMaze();

    ImGui::SetNextWindowPos(ImVec2(windowSize.x, windowSize.y), ImGuiCond_FirstUseEver);
    ImGui::SetNextWindowSize(windowSize, ImGuiCond_FirstUseEver);
    if (show_raycast)
        renderRayCast();
